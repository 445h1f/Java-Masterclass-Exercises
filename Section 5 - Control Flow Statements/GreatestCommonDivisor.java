...
public class GreatestCommonDivisor {
    ...
    public static int getGreatestCommonDivisor(int first, int second){
        ...
        // storing largest number for looping till largest number
        int largest;
        if (first > second){
            largest = first;
        }else{
        ...
        int gcd = 0; // to store common divisor (last assignment will be largest)

        for (int i=1; i<=largest; i++){
            // checking if i divides both first and second number
            if (first % i == 0 && second % i == 0){
                // updating gcd with common divisor i
                gcd = i;
            }
        }
