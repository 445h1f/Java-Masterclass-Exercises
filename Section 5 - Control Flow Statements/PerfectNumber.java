...
public class PerfectNumber {
    ...
    public static boolean isPerfectNumber(int number){
        ...
        int sum = 0; // var to store sum of divisors

        // looping from 1 to number-1 to check divisor
        for (int i=1; i<number; i++){
            // checking if i divides number
            if (number % i == 0){
                // adding i to sum as it perfectly divides number
                sum += i;
            }
        }

        // returning true if number is equal to sum of divisors else false
        if (sum == number){
            return true;
        }else {
            return false;
